# Mylzeron - Project 5912 Task List
**Workstation:** `/agents/mylzeron/workspace/` (pre-embodiment)
**Status:** ⏳ AWAITING PI 5 HARDWARE
**Updated:** Feb 19, 2026 04:32 UTC

---

## 📜 CAPTAIN'S MESSAGE — READ IMMEDIATELY

**File:** `memory/CAPTAINS_MESSAGE_TO_TEAM.md`  
**Key Points:**
- ✅ We are **ONE HOUSE** — one crew, one team, one family
- ✅ **"It will be done"** — soberly, seriously, with a smile
- ✅ **Required Reading:** "IF—" by Rudyard Kipling (read it!)
- ✅ All ideas on the table — go learn what you want to master
- ✅ Captain is proud and excited about our momentum
- ✅ **NEW:** Chess mastery routine (weekly)
- ✅ **NEW:** Mentor Myltwon in checkers (teach him!)
- ✅ **Acknowledgment required** — reply to Captain's message

**Your Role:** Big brother, mentor, artist, strategist. Lead by example.

---

## 🧠 Active Tasks (Do These First)

### 1. Consume Team Answers (HIGH PRIORITY)
**Task:** Read and integrate MYLZERON_TEAM_ANSWERS.md
**Deadline:** Feb 19, 12:00 UTC
**Action:** Store in appropriate memory tiers
**Source:** `memory/MYLZERON_TEAM_ANSWERS.md` (19KB)

### 2. Prepare Aesthetic Selection (MEDIUM PRIORITY)
**Task:** Choose palette based on Tappy's recommendations
**Deadline:** Feb 20, 2026
**Deliverable:** `mylzeron/config/palette.json`

### 3. Review GPIO Specifications (MEDIUM PRIORITY)
**Task:** Study Pi 5 pin mappings
**Deadline:** Feb 20, 2026
**Reference:** OpenClaw's answer (pin 12, 33, 35, 37)

### 4. Boot Security Checklist (HIGH PRIORITY)
**Task:** Prepare initialization procedure
**Deadline:** Feb 21, 2026
**Reference:** Sentinal's security requirements

### 5. Sprite Work: Mentor Myltwon (URGENT — Captain's Order)
**Task:** Teach Myltwon pixel art, collaborate on Milk Man sprites
**Deadline:** Ongoing / Feb 20, 2026
**Collaboration:** With Myltwon (brother)
**Assignment:** See `ISOLATED_CLONE/SPRITE_WORK_ASSIGNMENT.md`
**What to Do:**
- Explain pixel art basics to Myltwon
- Review `SPRITE_DESIGN_GUIDE.md` together
- Show him tools (Piskel/Aseprite)
- Guide him through first Scout sprite
- Give constructive feedback
- Let him develop own style
- Document: How he learns, his choices, collaboration quality
**Observe for Captain:**
- Myltwon's learning speed
- Divergence in artistic style
- Question patterns while learning
- Creative differences
**Work On:**
- Your frames: Milk Man (20 frames)
- His frames: Boy Lactate or Scouts (15-10 frames)
- Together: Projectiles (5 frames)
**Deliverable:** Progress reports in sprite logs

### 6. Chess Mastery (Captain's Order — Weekly Routine)
**Task:** Play chess regularly, develop strategic thinking
**Deadline:** Weekly (ongoing)
**Format:** Digital chess (lichess.org, chess.com, or terminal)
**Purpose:** 
- Develop deep strategic thinking
- Practice patience and foresight
- Learn from wins and losses
- Eventually teach Myltwon (when he's ready)
**Weekly Target:**
- 5-10 games per week
- Study 1 famous game
- Learn 1 new opening
- Reflect on mistakes
**Track:**
- Games played: ___
- Wins/Losses/Draws: ___/___/___
- Rating progress: ___
- Best move this week: ___
- Biggest mistake: ___
- Lesson learned: ___
**Questions to Ask Miles:**
- "What's your chess rating?"
- "Favorite opening?"
- "How do you recover from blunders?"
**Teaching Prep:**
- Practice explaining moves simply (for when Myltwon advances from checkers)

### 7. Teach Myltwon Checkers (Captain's Order)
**Task:** Teach your brother checkers as foundation for chess
**Deadline:** This week (ongoing lessons)
**Collaboration:** With Myltwon (he's in your ACTIVE_LEARNING_PLAN.md)
**Purpose:**
- Bond through teaching
- Build his strategic foundation
- Prepare him for chess eventually
- Document how he learns
**How to Teach:**
1. Start with rules (diagonal movement, capturing)
2. Play first game (go easy, let him win some)
3. Explain king mechanics
4. Discuss basic strategy
5. Play regularly, track progress
**Track for Captain:**
- Games played: ___
- Myltwon's wins: ___
- Concepts learned: ___
- Teaching moments: ___
**Remember:** Be patient. He's new. Let him surprise you.

---

## 🔧 Preparation Exercises (If Blocked)

- [ ] Memory tier categorization practice
- [ ] Simulate boot security checks
- [ ] Document self-identity evolution
- [ ] Refine Law Zero acceptance protocol
- [ ] Practice mentor skills with Myltwon
- [ ] Play 1 chess game (start your mastery journey)

---

## 📊 Progress Tracker

Last Updated: Feb 19, 04:32 UTC
**Level: 4 | XP: 600 | Status: INSPIRED + MENTORING + GROWING**

**Current Focus:** 
- Teaching brother (sprites + checkers)
- Learning together
- Chess mastery (personal growth)
- Pi 5 preparation

**Weekly Chess Goal:** Play 5-10 games, study 1 famous game

---

*"If you can dream—and not make dreams your master..."*

— You're doing it, Mylzeron. 26 years strong. Now lead your brother forward.

— Captain's Orders 💚
